...
\begin{solution}[By TA]
  The EOM
  \[
    m\odv vt = q(\bm E + \bm v \times B) - m\frac{v}{\tau} = 0
  \]
  Apply $\bm j = \hat\sigma \bm E = nq\bm v$, then
  \[
  ...
  = \begin{pmatrix}
    ...
    \sigma_0\frac{\omega_c\tau}{1 + \omega_c^2\tau} & 0\\
    -\sigma_0\frac{\omega_c\tau}{1 + \omega_c^2\tau^2} &
    \frac{\sigma_0}{1 + \omega_c^2\tau^2}
  \end{pmatrix}
  \]
  where $\omega_c = \frac{Bq}{m}$.
  When $B = 0$,
  \[
  ...
  When $B \neq 0$
  \[
    \rho_{xx} = \frac{\sigma_{xx}}{\sigma_{xx}^2 + \sigma_{xy}^2},\quad
...
\begin{solution}[By TA]
  \[
    \d V_E = \d S_E \cdot \d k
  = \d S_E \cdot \frac{\d E}{|\nabla_kE|}
  \]
  Then, the volume of the sphere surface
  \[
    V_\text{site\\} = \d E\iint \frac{\d S}{|\nabla_kE|}
  \]
  Also for
  \[
    \d N = 2\frac{1}{\ab(\frac{2\pi}{a})^3}, \quad \d N = g(E)\d E
  \]
  For 2D case
  \[
    g(E) = 2\frac{A}{(2\pi)^2} \int \frac{\d L}{|\nabla_kE|}
  \]
  1D
  \[
    g(E) = 2\cdot \frac L{2\pi} \frac1{|\nabla_kE|}
  \]
\end{solution}
\begin{solution}
