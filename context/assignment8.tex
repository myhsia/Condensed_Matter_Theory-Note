...
\paragraph{Homework \#8 Electrons in a magnetic field}
...
\begin{solution}\leavevmode
  \begin{enumext}
    ...
    motion becomes
    \[
      E_k = \frac{\hbar^2}{2m_{xx}}(k_x^2 + k_y^2), \quad
      \frac{k_x^2}{2m_{xx}E_k/\hbar^2} + \frac{k_y^2}{2m_{xx}E_k/\hbar^2} = 1.
    \]
    Substituting to the definition of cyclotron mass
    \[
      A = \pi (k_{x_0}^2 + k_{y_0}^2) = \frac{2\pi m_{xx} E_k}{\hbar^2},
      \enspace
      m_c = \frac{\hbar^2}{2\pi} \pdv A{E_k} = m_{xx}.
    \]
    ...
    \[
      E_k = \frac{\hbar^2 k_y^2}{2m_{xx}} + \frac{\hbar^2 k_z^2}{2m_{zz}},\quad
      \frac{k_y^2}{2m_{xx}E_k/\hbar^2} + \frac{k_z^2}{2m_{zz}E_k/\hbar^2} = 1.
    \]
    Similarly to (a)
    \[
      A = \pi k_{y_0} k_{z_0} = 2\pi E_k\sqrt{m_{xx}m_{zz}}/\hbar^2,
      \enspace
      m_c = \sqrt{m_{xx}m_{zz}}.
    \]
  \end{enumext}
\end{solution}
...
\begin{solution}\leavevmode
  \begin{enumext}
    ...
    \end{enumext}
    \item IQHE provides a quantized Hall resistance $R_H = h/e^2$,
...
\begin{problem}[Lifshitz-Kosevich formula]
  ...
  where $\lambda(T) = 2\pi^2 k_BTm^*/\hbar eB$.
  \begin{enumext}
...
\begin{solution}\leavevmode
  \begin{enumext}
    \item Take $\alpha = 2\pi^2k_Bm^*/\hbar eB$,
    then $\lambda(T) = \alpha T$.
    Since at $T \gg 1$, the
    amplitude $\Delta R$ satisfies
    \[
      \Delta R \propto R_T = \alpha T/\sinh(\alpha T)
      \to \alpha T \upe^{-\alpha T}/2, \
      \Delta R = \tilde C T\upe^{-\alpha T}.
    \]
    Take the logarithm to the two sides
    \[
      \ln(\Delta R/T) = -\alpha T + C,
    \]
    where $C = \ln\tilde C$.
    So, one can take plot the linear relation.
    By fitting the slope $k = -\alpha$ that fits the figure, one can obtain
    the effective mass
    \[
      m^* = \alpha \hbar eB/2\pi^2k_B
    = -k \hbar eB/2\pi^2k_B.
    \]
    ...
    = \tilde C \cdot \upe^{-(\alpha+\beta/\tau_q)\cdot\frac1B}/B,
    \end{multline*}
    ...
    sides, we can obtain
    \[
      \ln(\Delta R)  = C - \ln B - (\alpha + \beta/\tau_q)/B,
    \]
